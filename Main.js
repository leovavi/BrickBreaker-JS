let mainState = {
	...
	create: function(){
		...
		let pos3T, pos2T, brick3T, brick2T;
		currentLevel = 1;
	...
	startBricks: function(){
		...
		this.fn.startBricks(this.bricks, this.brickRows, this.brickCols, this.brickImgs, this.design, this.width);
	},
	...
	removeBrick: function(ball, brick){
		if(brick.key.includes(this.brickImgs[pos3T])){
			if(brick.key === this.brickImgs[pos3T]){
				brick.loadTexture(this.brickImgs[pos3T]+"2");
				sfx_breakBrick.play();
				return;
			}
			else if(brick.key === this.brickImgs[pos3T]+"2"){
				brick.loadTexture(this.brickImgs[pos3T]+"3");
				sfx_breakBrick.play();
				return;
			}
		}
		else if(brick.key === this.brickImgs[pos2T]){
			brick.loadTexture(this.brickImgs[pos2T]+"3");
			sfx_breakBrick.play();
			return;
		}
