let Lvl2 = {
	...
	create: function(){
		...
		this.powerUps = this.fn.locatePowerUps(this.design, this.brickRows, this.brickCols, 15, 7);
		...
		this.fn.addTimerDelay(this.timer, lvl2Music);
	...
	removeBrick: function(ball, brick){
		if(this.fn.removeBrick(ball, brick, this.powers, pos3T, pos2T)){
			...
			if(this.bricks.countLiving() == 0)
				this.fn.endGame(lvl2Music, this.timer);
		}
	},
	...
	loseLife: function(){
		...
		if(this.timer.running)
			this.fn.stopTime(this.timer, lvl2Music);
		if(lives === 0)
			this.fn.endGame(lvl2Music, this.timer);
	},

	power: function(player, power){
		newPlayerVelX = this.fn.power(player, power, this.ball, this.playerVelX, this.txtLives, this.timer, lvl2Music);
